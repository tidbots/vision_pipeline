...
class Yolo26Node:
    def __init__(self):
        ...
        self.input_topic = rospy.get_param(
            "~input_topic", "/camera/image_preprocessed"
        )
        self.det_topic = rospy.get_param(
            "~detections_topic", "/yolo26/detections"
        )
        self.ann_topic = rospy.get_param(
            "~annotated_topic", "/yolo26/image_annotated"
        )

        # debug
        self.debug_enable = rospy.get_param("~debug_enable", True)
        self.debug_topic = rospy.get_param(
            "~debug_topic", "/yolo26/image_debug"
        )
        ...
        self.pub_det = rospy.Publisher(
            self.det_topic, Detection2DArray, queue_size=1
        )
        self.pub_ann = rospy.Publisher(
            self.ann_topic, Image, queue_size=1
        )
        ...
        self.sub = rospy.Subscriber(
            self.input_topic, Image, self.cb, queue_size=1
        )
    ...
    def cb(self, msg):
        ...
        # ----------------------------
        # Debug image (tiles + HUD)
        # ----------------------------
        if self.debug_enable:
            dbg = ann.copy()
            for tid, x, y, w, h in tiles:
                cv2.rectangle(
                    dbg, (x, y), (x + w, y + h), (255, 0, 0), 1
                )
                cv2.putText(
                    dbg,
                    f"T{tid}",
            ...
            cv2.putText(
                dbg,
                f"FPS:{fps:.1f} tiles:{len(tiles)} det:{len(final_boxes)}",
                (10, 25),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (0, 255, 255),
